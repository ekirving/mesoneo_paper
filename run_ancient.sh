...
for chr in $(seq ${start} ${stop}); do
  for type in $types; do
    echo "Starting chr${chr} for ALL path(s) and ${type} SNPs..." >>$logfile
    (
      set -x
      snakemake ${flags} --config dataset=${dataset} population=${population} chr=${chr} ancestry=ALL type=${type} -- ancestries
    ) &>>${logfile}
  done
done
