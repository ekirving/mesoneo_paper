...
peaks.orig <- havester_to_bed(bind_rows(lapply(ancestries, function(ancestry) {
  read_tsv(paste0("clues/ancestral_paths_new-all-ancient-", ancestry, "-harvester.tsv"), col_types = cols()) %>%
...
peaks.post <- havester_to_bed(bind_rows(lapply(ancestries, function(ancestry) {
  read_tsv(paste0("clues/ancestral_paths_new-all-ancient-", ancestry, "-posterior_diff-harvester.tsv"), col_types = cols()) %>%
    mutate(ancestry = ancestry)
})))

# load all the refbias_Anc peaks
peaks.refbias_anc <- havester_to_bed(bind_rows(lapply(ancestries, function(ancestry) {
  read_tsv(paste0("clues/ancestral_paths_new-all-ancient-", ancestry, "-refbias_Anc-harvester.tsv"), col_types = cols()) %>%
    mutate(ancestry = ancestry)
})))
...
peaks.refbias_anc_1000g <- havester_to_bed(bind_rows(lapply(ancestries, function(ancestry) {
  read_tsv(paste0("clues/ancestral_paths_new-all-ancient-", ancestry, "-refbias_Anc_1000g-harvester.tsv"), col_types = cols()) %>%
    mutate(ancestry = ancestry)
})))
